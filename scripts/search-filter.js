...
document.addEventListener('DOMContentLoaded', (e) =>{
    ...
    let paginaAtual = 1;
    let paginaAntesDoClick = paginaAtual
...
function construirPagina(idPagina, dados){
    ...
    construirNavButtons(idPagina, dados.length)
}

function construirNavButtons(idPagina, numTotal){
    paginaAntesDoClick = idPagina
    navList.innerHTML = ''
    let rangeMin = 1
    ...
    let rangeMax = Math.min(rangeMin + 4,Math.ceil(numTotal/4))
    ...
    if (rangeMax < Math.ceil(numTotal/4)){
        navList.innerHTML += `<li class="bottom-nav__item"><a class="bottom-nav__link" href="#" id='+'>...</a></li>`
    }

    if (idPagina < Math.ceil(numTotal/4)){
        navList.innerHTML += `<li class="bottom-nav__item"><a class="bottom-nav__link" href="#" id='f'>&gt;&gt;</a></li>`
    }


    const navItems = navList.children
    console.log(navItems[0].firstChild)

    navItems[0].firstChild.addEventListener('click', (e)=>{
        e.preventDefault()
        console.log('clicou')
    })

    for (let i = 0; i < navItems.length; i++) {
        navItems[i].firstChild.addEventListener('click', (e)=>{
            e.preventDefault()
            ...
            if(e.target.id == 'f'){
                paginaAtual = Math.ceil(numTotal/4)
            }
